...
int main(int argc, char** argv) {
    ...
    if (provided < MPI_THREAD_FUNNELED) {
        std::cerr << "El nivel de soporte de hilos MPI es insuficiente." << std::endl;
...
#pragma omp for
        for (unsigned long long key = start_key; key < end_key; ++key) {
            if (!found && decrypt_and_search(key, text, keyword)) {
#pragma omp critical
                {
                    correct_key = key;
                    found = true;
...
#pragma omp cancel for
    ...
    if (global_found) {
        MPI_Allreduce(MPI_IN_PLACE, &correct_key, 1, MPI_UNSIGNED_LONG_LONG, MPI_MAX, MPI_COMM_WORLD);
    }
