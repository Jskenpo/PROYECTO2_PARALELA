...
unsigned long long dynamic_key_distribution(int world_rank, int world_size, unsigned long long key_range) {
    ...
    // Asegúrate de que el último proceso cubre el rango completo
...
int main(int argc, char** argv) {
    ...
    unsigned long long key_range = 100000000ULL; // Ajusta este rango según sea necesario
    unsigned long long start_key = dynamic_key_distribution(world_rank, world_size, key_range);

    std::cout << "Proceso " << world_rank << " buscando desde " << std::hex << start_key << " hasta " << std::hex << start_key + (key_range / world_size) << std::dec << std::endl;

    unsigned long long correct_key = 0;
    for (unsigned long long key = start_key; key < start_key + (key_range / world_size); ++key) {
